...
pub struct Virt2Boundary {
    routers: KRwLock<BTreeMap<RouterId, Arc<RouterV2b>>>,
}
...
pub struct RouterV2b {
    ...
    // optimized for fast query times. It's under an read-write lock to allow
    // multiple concurrent readers. When we update we hold the lock just long
    // enough to swap the entry with one that was pre-built out of band.
    pt4: KRwLock<Poptrie<Ipv4Cidr, BTreeSet<TunnelEndpoint>>>,
    pt6: KRwLock<Poptrie<Ipv6Cidr, BTreeSet<TunnelEndpoint>>>,
}
...
impl Virt2Boundary {
    ...
    pub fn router(&self, id: &RouterId) -> Option<Arc<RouterV2b>> {
...
impl RouterV2b {
    ...
    pub fn get_with_len(
        ...
        match vip {
            IpAddr::Ip4(ip4) => self
                ...
                .map(|(cidr, teps)| (cidr.prefix_len(), teps.clone())),
            IpAddr::Ip6(ip6) => self
                .pt6
                .read()
            ...
            IpCidr::Ip4(ip4) => {
                ...
                let (clear, orig) = match tbl.get_mut(&ip4) {
                    Some(entry) => {
                        ...
                        self.pt4.write().insert(ip4, entry.clone());
            ...
            IpCidr::Ip6(ip6) => {
                ...
                let (clear, orig) = match tbl.get_mut(&ip6) {
                    Some(entry) => {
                        ...
                        self.pt6.write().insert(ip6, entry.clone());
                        (entry.is_empty(), Some(orig))
                    }
                    None => (false, None),
                };
    ...
    pub fn set<I: IntoIterator<Item = TunnelEndpoint>>(
        ...
        match vip {
            IpCidr::Ip4(ip4) => {
                let mut tbl = self.ip4.lock();
                let updated = match tbl.get_mut(&ip4) {
                    Some(entry) => {
                        entry.extend(tep);
                        entry.clone()
                    }
                    None => {
                        let updated: BTreeSet<TunnelEndpoint> =
                            tep.into_iter().collect();
                        tbl.insert(ip4, updated.clone());
                        updated
                    }
                };
                self.pt4.write().insert(ip4, updated)
            }
            IpCidr::Ip6(ip6) => {
                let mut tbl = self.ip6.lock();
                let updated = match tbl.get_mut(&ip6) {
                    Some(entry) => {
                        entry.extend(tep);
                        entry.clone()
                    }
...
mod tests {
    ...
    fn lpm_wins_across_routers() {
        ...
        let list = RouterList::new([(10, r1), (20, r2)]).unwrap();
        let best = v2b.lookup_best(&list, &v4("10.1.2.3")).unwrap();
        assert_eq!(best, BTreeSet::from([tep(2)]));

        let best = v2b.lookup_best(&list, &v4("10.2.0.1")).unwrap();
        assert_eq!(best, BTreeSet::from([tep(1)]));
    }

    #[test]
    fn priority_breaks_lpm_ties() {
        let v2b = Virt2Boundary::new();
        let r1 = Some(Uuid::from_u128(1));
        let r2 = Some(Uuid::from_u128(2));
        v2b.set_for(r1, "10.0.0.0/8".parse().unwrap(), [tep(1)]);
        ...
        let list = RouterList::new([(20, r2), (10, r1)]).unwrap();
        let best = v2b.lookup_best(&list, &v4("10.1.2.3")).unwrap();
        assert_eq!(best, BTreeSet::from([tep(1)]));
    }
    ...
    fn unlisted_and_unknown_routers_are_ignored() {
        ...
        let list = RouterList::new([(10, r1), (20, ghost)]).unwrap();
        let best = v2b.lookup_best(&list, &v4("10.1.2.3")).unwrap();
        assert_eq!(best, BTreeSet::from([tep(1)]));

        // A list of only unknown routers matches nothing.
        let list = RouterList::new([(10, ghost)]).unwrap();
    ...
    fn router_list_rejects_duplicate_priorities() {
        let r1 = Some(Uuid::from_u128(1));
        let r2 = Some(Uuid::from_u128(2));
        assert!(RouterList::new([(10, r1), (10, r2)]).is_err());

        let list = RouterList::new([(20, r2), (10, r1)]).unwrap();
        assert_eq!(list.entries().collect::<Vec<_>>(), &[(10, r1), (20, r2)]);
    }
    ...
    fn router_list_rejects_duplicate_routers() {
        ...
        let err = RouterList::new([(10, r1), (20, r1)]).unwrap_err();
        assert!(err.contains("duplicate router"), "{err}");
        // The default router listed twice.
        let err = RouterList::new([(10, None), (20, None)]).unwrap_err();
        assert!(err.contains("duplicate default router"), "{err}");
        // Duplicates hidden among valid entries, in any order.
        assert!(
            RouterList::new([(30, r2), (10, None), (20, r1), (5, r2)]).is_err()
        );
        // Distinct routers, including one default entry, are fine.
        let list = RouterList::new([(20, r2), (1000, None), (10, r1)]).unwrap();
        assert_eq!(
            list.entries().collect::<Vec<_>>(),
            &[(10, r1), (20, r2), (1000, None)]
        );

        let err = RouterList::new([(10, r1), (10, None)]).unwrap_err();
        assert!(err.contains("duplicate priority"), "{err}");
    }

    #[test]
    fn router_list_rejects_oversized_lists() {
        let entry = |i: usize| (i as u16, Some(Uuid::from_u128(i as u128)));
        let max = crate::api::MAX_ROUTER_LIST_ENTRIES;
        assert!(RouterList::new((0..=max).map(entry)).is_err());
        assert!(RouterList::new((1..=max).map(entry)).is_ok());
    }
    ...
    fn empty_tep_sets_are_skipped() {
        ...
        let list = RouterList::new([(10, r1)]).unwrap();
        assert!(v2b.lookup_best(&list, &v4("10.1.2.3")).is_none());
    }
