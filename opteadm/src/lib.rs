...
#[derive(Debug, Error)]
pub enum Error {
    #[error("OPTE driver is not loaded")]
    DriverNotLoaded,

    #[error("Error interacting with driver device file: {0}")]
    Io(std::io::Error),
...
impl From<std::io::Error> for Error {
    fn from(e: std::io::Error) -> Error {
        match e.kind() {
            std::io::ErrorKind::NotFound => Error::DriverNotLoaded,
            _ => Error::Io(e),
        }
    }
}
...
/// A handle used to administer the Oxide Packet Transformation Engine (OPTE) driver.
#[derive(Debug)]
pub struct OpteAdm {
    device: File,
}
...
impl OpteAdm {
    pub const OPTE_DEV: &'static str = "/dev/opte";

    /// Create a new handle to the OPTE driver
    ...
    P: DeserializeOwned,
{
    ...
    for _ in 0..MAX_ITERATIONS {
        ...
        // Ioctl(2) failed for a reason other than the response buffer being too small.
        ...
        // At this point, the ioctl(2) failed because the buffer supplied for the serialized
        // response is not sufficient. Resize the buffer to that suggested by the driver, and retry
        // the ioctl.
    ...
    panic!("Failed to allocate sufficient ioctl(2) response buffer in {} iterations", MAX_ITERATIONS);
}
